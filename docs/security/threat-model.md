...
3. Container -> broker: protocolo HTTP v5 sobre Unix socket privado; o mount é read-only.
...
| Vazamento do login CLI | controle da conta Codex/Claude | auth dirs `0700`, `.claude.json` `0600`, home oculto por systemd, configuração Claude somente leitura no serviço e cópia efêmera na sandbox | check de capacidade, systemd, permissões e teste de cópia |
...
| Abuso de effort/modelo CLI | custo ou argumento inesperado | aliases fechados; modelo completo, effort efetivo e combinação revalidados no protocolo v5; argv reconstruído | testes de catálogo, protocolo, normalização e executor |
